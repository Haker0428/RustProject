...
impl ThreadPoolSharedData {
    ...
    fn no_work_notify_all(&self) {
        if !self.has_work() {
            *self.empty_trigger.lock().expect("Unable to notidy all joining threads");
...
impl Builder {
 ...
 fn spawn_in_pool(shared_data: Arc<ThreadPoolSharedData>) {
    ...
    builder.spawn(move || {
        ...
        loop {
            let thread_count_val = shared_data.active_count.load(Ordering::Acquire);
            let max_thread_val = shared_data.max_thread_count.load(Ordering::Relaxed);
            if thread_count_val > max_thread_val {
                break;
            }
